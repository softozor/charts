...
```sh
helm repo add nexus https://<username>:<password>@jelasticozor-pm.hidora.com/repository/helm-hosted
helm install my-release softozor/hasura
```

## Introduction

This chart bootstraps a Hasura deployment on a Kubernetes cluster using the Helm package manager. Ingress routes can be activated in enabling Traefik routes, that is ready to work with Let's Encrypt certificates and cert-manager.
...
```shell
helm install --name my-release softozor/hasura \
